...
def plot_output(die_size, cellLibrary, input_instances, output_instances, nets, placement_rows,inputPins,outputPins):
    ...
    for name, x, y in inputPins:
        plt.scatter(x, y, color='#008000',s = 3.7)
    print("check7")

    for name, x, y in outputPins:
        plt.scatter(x, y, color='navy',s = 3.7)
    print("check8")
    ...
    for instance in input_instances.items():#plot gate only
        if(instance[1][3] == True):
            ...
            rect = plt.Rectangle((inst_x, inst_y), cell_width, cell_height, fill=False, edgecolor='black')
    ...
    num_instance_plotted = 0
    # for inst_name in output_instances.items():
    #     inst_type = inst_name[1][0]
    #     inst_x = inst_name[1][1]
    #     inst_y = inst_name[1][2]
    #     cell_width = cellLibrary[inst_type][0]
    #     cell_height = cellLibrary[inst_type][1]
    #     num_of_bit = cellLibrary[inst_type][4]
    #     if(num_of_bit == 1):
    #         color = 'skyblue'
    #     elif(num_of_bit == 2):
    #         color = 'green'
    #     elif(num_of_bit == 4):
    #         color = 'violet'
    #     rect = plt.Rectangle((inst_x, inst_y), cell_width, cell_height, fill=False, edgecolor=color)
    #     plt.gca().add_patch(rect)
    #     num_instance_plotted += 1
    print('number of instance   ',num_instance_plotted)
